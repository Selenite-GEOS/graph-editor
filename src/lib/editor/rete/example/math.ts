...
export const sumExample: EditorExample = async (factory: NodeFactory) => {
  ...
  await editor.addConnection(
    new Connection(numberNode, "value", addNode, "left"),
  );
  await editor.addConnection(
    new Connection(numberNode2, "value", addNode, "right"),
  );

  const displayNode = new DisplayNode({ factory, initial: 3 });
  await editor.addNode(displayNode);
  await editor.addConnection(
    new Connection(addNode, "value", displayNode, "input"),
  );
  await editor.factory?.arrange?.layout();
  const area = editor.factory?.getArea();
  if (area) {
