...
export class NodeEditor extends BaseNodeEditor<Schemes> {
  ...
  setName(name: string, triggerListeners = true) {
    ...
    if (triggerListeners)
      this.onChangeNameListeners.forEach((listener) => listener(this.name));
  }
  name = "New Editor";
  ...
  async addExecConnection(source: Node, target: Node): Promise<boolean> {
    return await this.addConnection(
      new Connection(source, "exec", target, "exec"),
    );
  }
  ...
  async addNewConnection(
    ...
    return await this.addConnection(
      new Connection(source, sourceOutput, target, targetInput),
    );
  }
