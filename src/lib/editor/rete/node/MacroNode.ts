...
export class MacroNode extends Node {
  ...
  constructor({
    ...
    super({
      factory,
      params: { saveData, graphId, graphVersion },
      height: 60,
      width: 200,
    });
    ...
    this.macroEditor.addPipe(async (ctx) => {
      if (ctx.type === "connectioncreate") {
        ...
        const inputSocket = this.macroEditor.getNode(ctx.data.target).inputs[
          ctx.data.targetInput
        ]?.socket;
        if (outputSocket?.isArray === true && inputSocket?.isArray === true) {
          if (ctx.data.targetInput in inputSocket.node.ingoingDataConnections) {
            for (const conn of inputSocket.node.ingoingDataConnections[
              ctx.data.targetInput
            ]) {
  ...
  async initialize(params: {
          ...
          options: {
            ...
            change: (value) => {
              ...
              wu(this.macroEditor.getNodes())
                .filter(
                  (n) => n instanceof VariableNode && n.variableId === v.id,
                )
                .forEach((n) => {
    ...
    for (const node of this.macroEditor.getNodes()) {
      for (const [key, input] of Object.entries(
        node.socketSelectionComponent.selectedInputs(),
      )) {
        ...
        if (isMicroMacroInput) {
          ...
          while (microMacroNodeIds.length > 0) {
            ...
            microMacroNode = microMacroNode.macroFactory.getNode(
              microMacroNodeId,
            ) as MacroNode;
          }
          // console.log("loop microMacroNode", microMacroNode)
          ...
          console.log(
            "Deleting micro macro input",
            microKey + "¤" + microNodeId,
          );
          delete microMacroNode.macroInputs[key];
          ...
          await this.macroEditor.addNewConnection(
            inputNode,
            "value",
            node,
            key,
        ...
        const baseInputControl = input.control as
          | InputControl<"text">
          | undefined;
          ...
          const execNode = await this.macroFactory.addNode(OutExecNode, {
            onExecute: async () => {
              if (macroKey in this.outgoingExecConnections) {
                ...
                const promises = this.getWaitPromises(
                  getLeavesFromOutput(this, macroKey),
                );
                this.factory
                  .getControlFlowEngine()
                  .execute(conn.target, conn.targetInput);
  ...
  override async data(
    ...
    for (const outputKey of Object.keys(this.outputs)) {
      const [microOutputKey, microNodeId, ...microMacroNodeIds] =
        outputKey.split("¤");
