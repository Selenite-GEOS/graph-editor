  ...
  implements DataflowNode, ComponentSupportInterface
{
  ...
  outputs: {
    [key in keyof Outputs]?:
      | Output<Exclude<Outputs[key], undefined>>
      | undefined;
  } = {};
  ...
  constructor(params: NodeParams) {
    ...
    this.socketSelectionComponent =
      this.addComponentByClass(R_SocketSelection_NC);
    this.state = {};
    Node.nodeCounts++;
  ...
  getOutgoers(key: string): Node[] | null {
    if (key in this.outgoingExecConnections) {
      return this.outgoingExecConnections[key].map((conn) =>
        this.getEditor().getNode(conn.target),
      );
    } else if (key in this.outgoingDataConnections) {
      return this.outgoingDataConnections[key].map((conn) =>
        this.getEditor().getNode(conn.target),
      );
    }
    return null;
  ...
  addInExec(name = "exec", displayName = "") {
    const input = new Input(
      new ExecSocket({ name: displayName, node: this }),
      undefined,
      true,
    );
    this.addInput(
      name,
      input as unknown as Input<Exclude<Inputs[keyof Inputs], undefined>>,
    );
  }
