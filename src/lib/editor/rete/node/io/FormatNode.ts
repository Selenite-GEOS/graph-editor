  ...
  getValues(inputs: Record<string, unknown[]>) {
    ...
    Object.keys(this.inputs).forEach((key) => {
      if (key.startsWith("data-"))
        values[key.slice("data-".length)] = this.getData(key, inputs);
    });
    return values;
  }

  override data(inputs?: Record<string, unknown[]> | undefined): {
    result?: string;
  } {
    const res = { result: "" };
    ...
    try {
      res.result = getMessageFormatter(
        this.getData<"text">("format", inputs) as string,
