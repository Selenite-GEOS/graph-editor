    ...
    const comment = new FrameComment(params.text, this.area, this.editor, {
      contextMenu: ({ id }) => {
        ...
        spawnMoonMenu({
          searchbar: false,
          pos: {
            x: this.lastPointerEvent?.clientX ?? 0,
            y: this.lastPointerEvent?.clientY ?? 0,
          },
          items: [
            ...
            createActionMenuItem({
              ...
              executeAction: () => {
                ...
                this.factory.history?.add({
                  redo: () => this.delete(id),
                  undo: () =>
                    this.addFrame(comment.text, comment?.links, { id }),
                });
                this.delete(id);
              },
            }),
      ...
      translate: async ({ id }, dx, dy, sources) => {
        ...
        if (comment.text === "") {
          comment.nested.textContent = get(_)(
            "graph-editor.comment.default-text",
          );
        }
      ...
      response: (r) => {
        ...
        const redo = () => {
          ...
          comm.nested.textContent =
            r !== "" ? r : get(_)("graph-editor.comment.default-text");
        ...
        this.factory.history?.add({
          ...
          undo: () => {
            ...
            comm.nested.textContent =
              oldText !== ""
                ? oldText
                : get(_)("graph-editor.comment.default-text");
          },
        });
        redo();
      },
