...
export function socketTypeExport(t: SocketType): ExportedSocketType {
  if ((baseSocketTypes as readonly string[]).includes(t))
    return t as BaseSocketType;
...
export function isConnectionInvalid(outputSocket: Socket, inputSocket: Socket) {
  ...
  if (inType && outType && inType === outType) {
    ...
    const intersection = outSubtypesArray.filter((subtype) =>
      inSubtypesArray.includes(subtype),
    );
  ...
  return (
    ...
    (outputSocket.type !== inputSocket.type &&
      ...
      !(
        outputSocket.type === "pythonObject" &&
        inputSocket.type === "pythonProperty"
      ) &&
      !(
        outputSocket.type === "pythonProperty" &&
        inputSocket.type === "pythonObject"
      ))
  );
}

export class TypedSocketsPlugin<Schemes extends BaseSchemes> extends Scope<
  never,
  [Root<Schemes>]
> {
  ...
  setParent(scope: Scope<Root<Schemes>>): void {
    ...
    this.addPipe(async (ctx) => {
      ...
      if (ctx.type === "connectioncreate" && (conn = ctx.data as Connection)) {
        const outputSocket = this.getOutputSocket(
          conn.source,
          conn.sourceOutput,
        );
        ...
        if (isConnectionInvalid(outputSocket, inputSocket)) {
          ...
          if (
            (nodeEditor = scope as NodeEditor<Schemes>) &&
            this.lastConnectionRemove
          ) {
            console.log(
              `Restoring connection ${this.lastConnectionRemove?.id}`,
            );
