...
export class PythonNodeComponent extends NodeComponent {
  static isDynamicInput(
    inputs: Record<string, PythonComponentData[]>,
  ): boolean {
...
`;
    ...
    if (typeof data === "object") {
      if (Array.isArray(data)) {
        ...
        return `{${Object.entries(data)
          .map(
            ([key, value]) =>
              `${key}: ${PythonNodeComponent.toPythonData(value)}`,
          )
          .join(", ")}}`;
      }
    }
  ...
  async fetch(): Promise<Record<string, PythonComponentData>> {
    return await this.node
      .getFactory()
      .pythonDataflowEngine.fetch(this.node.id);
  }
  ...
  async fetchInputs(): Promise<Record<string, PythonComponentData[]>> {
    try {
      return await this.node
        .getFactory()
        .pythonDataflowEngine.fetchInputs(this.node.id);
    } catch (e) {
      const firstMatch = /"(.*?)"/.exec(
      ...
      if (firstMatch) {
        ...
        console.error(
          "Problematic node",
          this.node.getFactory().getEditor().getNode(nodeId),
        );
      }
  ...
  private async formatPythonVars(
    template: string,
    inputs?: Record<string, PythonComponentData[]>,
  ) {
    const varPattern = /([^$]*)\$(\(([^)]+)\)|\[([^]+)\])([^$]*)|([^]+)/g;
  ...
  static async collectPythonData(
    ...
    let resImportsStatements: Set<string> =
      node.pythonComponent.importsStatements;
    let resClasses: Record<string, string> = node.pythonComponent.classes;
    let resInitCode: string[] =
      nodeInput !== "exec"
        ? []
        : await Promise.all(
            node.pythonComponent.initCode.map((code) =>
              node.pythonComponent.formatPythonVars(code),
            ),
          );
    let resParserArguments: Map<string, ParseArgumentData> =
      node.pythonComponent.parseArguments;
        ...
        templateVars[key] = (
          await Promise.all(
            node.pythonComponent.code.map(
              async (code) =>
                childIndentation +
                (await node.pythonComponent.formatPythonVars(code)),
            ),
          )
        ).join("\n");
      } else {
